    ...
    <Box sx={{ display: "flex" }}>
      {isMobile ? (
        <Drawer
          ...
          PaperProps={{ sx: { width: 200, background: "var(--background-light)", borderRadius: "var(--border-radius-medium)", margin: "8px", border: "1px solid var(--border-color)" } }}
        >
          <List className={styles.sidebar}>
            ...
            >
              <SchoolIcon sx={{fontSize: "35px", color: "var(--neon-green)"}} /> LMS
            </Typography>
            {renderMenuItems()}
          </List>
        </Drawer>
      ...
      <AppBar
        ...
        sx={{
          ml: isMobile ? 0 : 300, /* Adjusted for sidebar width */
          width: isMobile ? "100%" : "calc(100% - 300px)", /* Adjusted for sidebar width */
        }}
      >
        <Toolbar className={styles.toolbar}>
          ...
          <IconButton onClick={handleUserMenu} className={styles.iconBtn}>
            <AccountCircle sx={{ color: "var(--neon-green)" }} fontSize="large" />
          </IconButton>
          <Menu
            anchorEl={anchorEl}
